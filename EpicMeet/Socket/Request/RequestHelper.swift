...
//                sendCreateRoomRequest(roomId: roomId)
        print("roomid in the request helper which is passed:",roomId)
...
//    }
    ...
    public func createWebRtcTransport(transportTypeStr: String,deviceObj: MediasoupDevice) -> JSON
    {
        guard let socket = self.socket else { return JSON() }
    ...
    public func connectTransport(transportTypeStr: String,transportID: String,dtlsParameters: JSON,role: String) -> JSON
    {
        guard let socket = self.socket else { return JSON() }
    ...
    public func getProducerList()
    {
        ...
        message.sendData(method: createWebrtcTransportRequest)
    }
    ...
    func onProduceCallBack(transportId:String,kind:String,rtpParameters:String)->JSON{
        ...
        print("ProduceJSON :",createWebrtcTransportRequest)
        let returnadata = message.sendData(method: createWebrtcTransportRequest)
        print("RTP CAPABLITIES\(returnadata)")
        let json:JSON = JSON(returnadata)
        return createWebrtcTransportRequest
...
extension RequestHelper:WebSocketDelegate{
    ...
    func onDidReceiveMessage(message:[String:Any]) {
        ...
        {
            joinRoomRequest(roomId: roomID, name: nameID, userId: userID)
//            createWebRtcTransport(roomid: roomId, transportTypeStr: "createWebRtcTransport", deviceObj: self.device!)
        ...
        if Event == "RtpCapabilitiesReceived"
        {
            ...
            self.device = device
            createWebRtcTransport(transportTypeStr: "producerTransport", deviceObj: self.device!)
        ...
        if Event == "CreateWebRtcTransportSuccess"
        {
            ...
            if transportTypeSTR == "producerTransport"
            {
                ...
                createWebRtcTransport(transportTypeStr: "consumerTransport", deviceObj: self.device!)
//                startVideoAndAudio() to be uncommented
...
//            print(userid)
//            acknowledgeUserStatus(userId: userid)
        }
        ...
        if Event == "UserLoginSuccess"
        {
            ...
            let NewClientId = dataArray["clientID"] as! String
            print(userid)
...
//                dashboard.dashBoardTableView.reloadData()
//            }
...
//            for items in dataArray
//            {
//                let joinedUserArray = items["joinedusers"] as! [[String:Any]]
//                print("joinedUsersArray :\n",joinedUserArray)
//                print("joined users array counnt is :",joinedUserArray.count)
//            }
