       ...
       // let rtpDic = rtpParameters.toDic()
        if self.isAudioLaunchFinish && self.isVideoLaunchFinish {
           ...
           // self.helper.onConnectCallBack(transportId:transportId, dtlsParameters:self.dtlsParameters)
                 ...
                 self.helper.connectTransport(transportTypeStr: "Producer", transportID: transport.getId(), dtlsParameters: JSON(rawValue: self.dtlsParameters.toDic()) ?? "", role: "server")
...
//        DispatchQueue.global().asyncAfter(deadline: DispatchTime.now()+2.5) {
        ...
        self.helper.connectTransport(transportTypeStr: "Consumer", transportID: id, dtlsParameters: JSON(rawValue: dtlsParameters.toDic()) ?? "", role: "client")
